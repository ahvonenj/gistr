...
var importDatabase = function(argv)
{
	if(argv.id)
	{
		pullGistById(argv.id, argv);
	}
	else if(argv.f || argv.file)
	{
		var file = argv.f || argv.file || process.cwd() + '/gistr-db-export.gs';
	}
	else
	{
		return console.log('Command needs to be called with --id="<gist id>" or --file="<pathToFile>"');
	}
}
