		...
		}, function(err)
		{
			...
			else
			{
				...
				function(data, response)
				{
					...
					fs.writeFile(process.cwd() + '/resp.json', JSON.stringify(gists), function(err)
					{
						if(err) throw err;
					});
				})
...
// Storage functions
var addStorage = function(key, item)
{
	store.put(key, item);
}
...
var updateStorage = function(key, newitem)
{

}

var addStorageDebug = function(argv)
{
	store.put(argv.key, argv.value);
}

var getStorageDebug = function(argv)
{
	return console.log(store.get(argv.key));
}
...
// Storage
//exports.addstorage = addStorageDebug;
//exports.getstorage = getStorageDebug;